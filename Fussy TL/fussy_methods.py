...
def above_average_filters(filters_miou, leave_factor=None, filters=None, last=False):
    """Returns indices of filters with above average/set factor IoU"""
    if leave_factor == None:
        out = []
        ...
        indexes = list(range(len(list_copy)))
        # sort indexes list
        indexes = [x for _, x in sorted(zip(list_copy, indexes))]
        # get index of last viable item
        last_index = round((len(list_copy) - 1) * leave_factor)
        indexes.reverse()
        out = indexes[:last_index]
        out.sort()
        return out
...
########################################################################################################################
##########################################FUSSY METHODS FOR COMBINED MODEL##############################################
########################################################################################################################

def predict_joined(model, data_tuple):
    """Returns prediction from data tuple in correct size as 2 element list"""
    image = cv2.imread(data_tuple[0])
    segmented = cv2.imread(data_tuple[1])
    # check if segmented is 0-1 or 0-255 and leave as 0-255
    if segmented.max() < 127:
    ...
    for i in range(0, prediction.shape[-1]):
        prediction_out.append(prediction[0, :, :, i])
    # get prediction dimensions for segmented resize
    dims = prediction.shape[1:-1]
    # resize segmented
    segmented = cv2.resize(segmented, dims)
    # return joined prediction
    return [prediction_out, segmented]


def compute_out_matrix_c(data_list, model):
    """return complete output matrix for
    selecting best candidate kernels
    with output dimensions:
    images x filters"""
    out = np.empty((len(data_list), model.outputs[0].shape[-1]), dtype='float32')
    # compute prediction for every image
    for i in range(0, len(data_list)):
        prediction = predict_joined(model, data_list[i])
        #compute IoU for every prediction
        for j in range(0, len(prediction[0])):
            iou = IoU_compute(prediction[0][j], prediction[1])
            out[i, j] = iou
...
def compute_out_matrix_predicate_test(data_list, model):
    """return complete output matrix for
    selecting best candidate kernels
    with output dimensions:
    images x filters - FOR PREDICATE TESTING"""
    out = np.empty((len(data_list), model.outputs[0].shape[-1]), dtype='float32')
    # compute prediction for every image
    for i in range(0, len(data_list)):
        prediction = predict_joined(model, data_list[i])
        #compute IoU for every prediction
        for j in range(0, len(prediction[0])):
            iou = t_predicate2(prediction[0][j], prediction[1])
            out[i, j] = iou
    return out

def filters_to_remove(filters, above_avg_filters):
    """Returns list of liters to remove"""
    initial_filters_indexes = list(range(0, filters))
    for above_filter in above_avg_filters:
        initial_filters_indexes.remove(above_filter)
    initial_filters_indexes.sort()
    initial_filters_indexes.reverse()
    return initial_filters_indexes

def purge_kernels(layer, filters_to_remove):
    """Returns list containing new conv with purged kernels
    and [filters, bias] list for setting weights"""
    filters = layer.get_weights()[0]
    bias = layer.get_weights()[1]
    for filter in filters_to_remove:
        filters = np.delete(filters, filter, 2)
    # make new purged conv
    new_conv = Conv2D(len(bias), layer.kernel_size, padding='same', activation='relu', strides=layer.strides)
    return [new_conv, [filters, bias]]

def t_predicate1(image, segment):
    """compute IoU with prediction and segment"""
    # make sure prediction is in uint8
    image = image.astype('uint8')
    intersection = cv2.bitwise_and(image, segment)
    out = np.count_nonzero(intersection) / np.count_nonzero(segment)
    return out

def t_predicate2(image, segment):
    """compute IoU with prediction and segment"""
    # make sure prediction is in uint8
    image = image.astype('uint8')
    intersection = cv2.bitwise_and(image, segment)
    inverted_int = cv2.bitwise_and(cv2.bitwise_not(image), cv2.bitwise_not(segment))
    # account for 0 division
    try:
        active = np.count_nonzero(intersection) / np.count_nonzero(segment)
    except ZeroDivisionError:
        active = 1
    try:
        inactive = np.count_nonzero(inverted_int) / np.count_nonzero(cv2.bitwise_not(segment))
    except ZeroDivisionError:
        inactive = 1
    out = (active + inactive) / 2
    return out
...
def copy_model(model):
    ...
    for layer in model.layers:
        out.add(layer)
    #out.build(input_shape=(1, 224, 224, 3))
    return out

def merge_lists(filters_1, filters_2):
    """Merges 2 lists of filters into single one without repetitions"""
    out = list(set(filters_1 + filters_2))
    out.sort()
    return out
